    ...
    <div class="row">
      <div class="col-lg-9">
        <div class="card-tools pull-right">
          <a href="{{route('crear_prescriptor')}}" class="btn btn-default border-info">
            <i class="fas fa-plus-circle pr-2"></i>Nuevo
          </a>
        </div>
      </div>
      <div class="col-lg-3">
        <form class="form-horizontal" method="POST" action="{{route('prescriptor')}}">
          @csrf
    ...
    <div class="card mt-2">
      ...
      <div class="card-body table-responsive p-0">
        <table class="table table-hover" id='tabla-data'>
          ...
          <tbody class="border-bottom">
            ...
            <tr>
              <td>{{number_format($prescriptor->RUTPre, 0, ",", ".")}}-{{$prescriptor->DVPre}}</td>
              <td>{{$prescriptor->NomPre}}</td>
              <td style="border: none !important;">
                <a href="{{route('editar_prescriptor', ['IdPre' => rtrim($prescriptor->IdPre)])}}"
                  class="btn-accion-tabla tooltipsC" title="Editar este registro">
                  <i class="fas fa-pencil-alt icon-circle bg-info"></i>
                </a>
                <form action="{{route('eliminar_prescriptor', ['IdPre' => rtrim($prescriptor->IdPre)])}}"
      ...
      <div class="card-footer">
        <div class="dataTables_paginate paging_simple_numbers float-right">
          {{$prescriptores->links("pagination::bootstrap-4")}}
        </div>
      </div>
      <!-- /.card-footer-->
    </div>
