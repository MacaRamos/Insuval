    ...
    <div class="row">
      <div class="col-lg-9">
        <div class="card-tools pull-right">
          <a href="{{route('crear_articulo')}}" class="btn btn-default border-info">
            <i class="fas fa-plus-circle pr-2"></i>Nuevo
          </a>
        </div>
      </div>
      <div class="col-lg-3">
        <form class="form-horizontal" method="POST" action="{{route('articulos')}}">
          @csrf
      ...
      <div class="card-body table-responsive p-0">
        <table class="table table-hover" id='tabla-data'>
          ...
          <tbody class="border-bottom">
            ...
            <tr>
              <td>{{$articulo->Art_nom_ex}}</td>
              <td>{{$articulo->Art_serie}}</td>
              <td>{{$articulo->Art_fecVenc}}</td>
              <td style="border: none !important;">
                <a href="{{route('editar_articulo', ['Art_cod' => rtrim($articulo->Art_cod)])}}"
                  class="btn-accion-tabla tooltipsC" title="Editar este registro">
                  <i class="fas fa-pencil-alt icon-circle bg-info"></i>
                </a>
                <form action="{{route('eliminar_articulo', ['Art_cod' => rtrim($articulo->Art_cod)])}}"
