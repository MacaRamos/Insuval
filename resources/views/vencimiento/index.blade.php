    ...
    <div class="row">
      <div class="col-lg-9">
        <div class="card-tools pull-right">
          <a href="{{route('crear_vencimiento')}}" class="btn btn-default border-info">
            <i class="fas fa-plus-circle pr-2"></i>Nuevo
          </a>
        </div>
      </div>
      <div class="col-lg-3">
        <form class="form-horizontal" method="POST" action="{{route('vencimiento')}}">
          @csrf
      ...
      <div class="card-body table-responsive p-0">
        <table class="table table-hover" id='tabla-data'>
          ...
          <tbody class="border-bottom">
            ...
            <tr>
              <td>{{$vencimiento->Ven_cantidad}}</td>
              <td>{{$vencimiento->Ven_tipo}}</td>
              <td style="border: none !important;">
                <a href="{{route('editar_vencimiento', ['Ven_codigo' => rtrim($vencimiento->Ven_codigo)])}}"
                  class="btn-accion-tabla tooltipsC" title="Editar este registro">
                  <i class="fas fa-pencil-alt icon-circle bg-info"></i>
                </a>
                <form action="{{route('eliminar_vencimiento', ['Ven_codigo' => rtrim($vencimiento->Ven_codigo)])}}"
