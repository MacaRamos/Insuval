    ...
    <div class="col-lg-12">
        ...
        <div class="card mt-2">
            <div class="card-header with-border border-bottom-3 border-info">
                <h3 class="card-title">Vencimiento</h3>
                <div class="card-tools pull-right">
                    <a href="{{route('vencimiento')}}" class="btn btn-block btn-info btn-sm ">
                        <i class="fas fa-reply"></i> Volver a listado
                    </a>
                </div>
            </div>
            <!-- form start -->
            <form action="{{route('actualizar_vencimiento', ['Ven_codigo' => trim($vencimiento->Ven_codigo)])}}" id="form-general" class="form-horizontal" method="POST"
                autocomplete="off">
                @csrf @method('put')
                <div class="card-body">
                    @include('vencimiento.form')
                </div>
                <!-- /.card-body -->
